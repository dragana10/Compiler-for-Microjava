...
public class SemanticPass extends VisitorAdaptor {
	 ...
	 * report_info("Deklarisana promenljiva "+ varDecl.getVarName(), varDecl); Obj
	 * varNode = Tab.insert(Obj.Var, varDecl.getVarName(),
	 * varDecl.getType().struct); }
	 * 
	 * public void visit(PrintStmt print) { printCallCount++; }
	 * 
	 * public void visit(ProgName progName){ progName.obj = Tab.insert(Obj.Prog,
	 * progName.getProgName(), Tab.noType); Tab.openScope(); }
	 * 
	 * public void visit(Program program){ nVars = Tab.currentScope.getnVars();
	 * Tab.chainLocalSymbols(program.getProgName().obj); Tab.closeScope(); }
	 * 
	 * public void visit(Type type){ Obj typeNode = Tab.find(type.getTypeName());
	 * if(typeNode == Tab.noObj){ report_error("Nije pronadjen tip " +
	 * type.getTypeName() + " u tabeli simbola! ", null); type.struct = Tab.noType;
	 * }else{ if(Obj.Type == typeNode.getKind()){ type.struct = typeNode.getType();
	 * }else{ report_error("Greska: Ime " + type.getTypeName() +
	 * " ne predstavlja tip!", type); type.struct = Tab.noType; } } }
	 * 
	 ...
	 * Tab.find(designator.getName()); if(obj == Tab.noObj){
	 * report_error("Greska na liniji " + designator.getLine()+
	 * " : ime "+designator.getName()+" nije deklarisano! ", null); } designator.obj
	 * = obj; }
	 * 
	 * 
	 ...
	 * 
	 * public void visit(AddExpr addExpr){ Struct te = addExpr.getExpr().struct;
	 * Struct t = addExpr.getTerm().struct; if(te.equals(t) && te == Tab.intType){
	 * addExpr.struct = te; }else{ report_error("Greska na liniji "+
	 * addExpr.getLine()+" : nekompatibilni tipovi u izrazu za sabiranje.", null);
	 ...
	 * currMethType = currentMethod.getType();
	 ...
	 * if(!assignment.getExpr().struct.assignableTo(assignment.getDesignator().obj.
